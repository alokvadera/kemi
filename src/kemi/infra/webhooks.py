...
if TYPE_CHECKING:
    from kemi.infra.encryption import FernetEncryptor
...
class WebhookEventType(str, Enum):
    ...
    def from_string(cls, value: str) -> "WebhookEventType":
        """Parse a string to an event type, raising ValidationError on failure."""
        try:
            return cls(value)
        except ValueError as exc:
            valid = ", ".join(e.value for e in cls)
            raise ValidationError(f"Invalid event type '{value}'. Valid: {valid}") from exc


def validate_webhook_url(url: str) -> None:
    """Validate a webhook URL for SSRF safety.

    Rejects non-HTTP(S) schemes, localhost, and private/reserved/link-local
    IP ranges. Raises ValidationError on failure.
    """
    parsed = urlparse(url)
    if parsed.scheme not in {"http", "https"}:
        raise ValidationError("Webhook URL scheme must be http or https")
    ...
    if hostname_lower in {"localhost", "127.0.0.1", "0.0.0.0", "::1", "[::1]"}:
        raise ValidationError("Webhook URL must not point to localhost")
    try:
...
def _get_secret_encryptor() -> "FernetEncryptor | None":
    """Return a FernetEncryptor if ``KEMI_ENCRYPTION_KEY`` is set, else None."""
    try:
        from kemi.infra.encryption import FernetEncryptor
    except Exception:
        return None
...
class WebhookStore:
    ...
    When ``KEMI_ENCRYPTION_KEY`` is set, webhook secrets are encrypted at
    rest with Fernet.  If the key is absent, secrets are stored as
    plaintext and a warning is logged once per process.
    """

    _warned_plaintext: bool = False

    def __init__(self, db_path: str) -> None:
        self._db_path = db_path
        self._encryptor = _get_secret_encryptor()
        if self._encryptor is None and not WebhookStore._warned_plaintext:
            WebhookStore._warned_plaintext = True
            logger.warning(
                "Webhook secrets are stored in plaintext because "
                "KEMI_ENCRYPTION_KEY is not set. Set the environment variable "
    ...
    def _encrypt_secret(self, secret: str) -> str:
        if not secret or self._encryptor is None:
            return secret
        return self._encryptor.encrypt(secret)

    def _decrypt_secret(self, ciphertext: str) -> str:
        if not ciphertext or self._encryptor is None:
            return ciphertext
        try:
            return self._encryptor.decrypt_str(ciphertext)
        except Exception:
            # Graceful fallback for rows created before encryption was
            # enabled (plaintext secret).
            return ciphertext
    ...
    def _row_to_config(self, row: sqlite3.Row) -> WebhookConfig:
        ...
        return WebhookConfig(
            ...
            secret=self._decrypt_secret(row["secret"]),
