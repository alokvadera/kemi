...
def memory_to_dict(memory: MemoryObject) -> dict[str, Any]:
    """Convert a :class:`MemoryObject` to a JSON-serialisable dict.

    Used for webhook payloads, audit log entries, and import/export
    payloads. Lives on :mod:`kemi.models` so both
    :mod:`kemi.pipeline.ingestion` and :mod:`kemi.operations._io` can
    ...
    return {
        ...
        "last_accessed_at": memory.last_accessed_at.isoformat() if memory.last_accessed_at else None,  # noqa: E501
