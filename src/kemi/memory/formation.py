    ...
    def extract(
        ...
        *,
        ...
        try:
            ...
            # Distinguish caller-configuration errors (bad key, no network)
            # from runtime failures so the user knows *why* extraction failed.
            err_msg = str(exc).lower()
            if "authentication" in err_msg or "api key" in err_msg:
                logger.error("OpenAI authentication failed: %s", exc)
                raise ConfigurationError(
                    "Invalid OpenAI API key. Set it explicitly or via OPENAI_API_KEY."
                ) from exc
        ...
        try:
            ...
            logger.warning(
                "OpenAI response was not valid JSON (%s). Raw prefix: %r",
                exc,
                raw[:200],
            )
            return []
