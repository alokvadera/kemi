...
from kemi.models import LifecycleState, MemorySource, MemoryType
from kemi.webhooks import WebhookConfig, WebhookEventType, WebhookStore, RetryConfig
...
try:
    ...
    BaseModel = object  # type: ignore[no-redef,assignment]
...
def _api_key_required() -> bool:
    ...
    return os.environ.get("KEMI_API_KEY_REQUIRED", "false").lower() in ("true", "1", "yes")
...
def create_app(memory: Memory | None = None) -> Any:
    ...
    async def api_key_middleware(request: Request, call_next: Any) -> Any:
        ...
        if header:
            ...
            if manager is None:
                raise HTTPException(
                    status_code=501,
                    detail="API key authentication not supported by this storage",
                )
    ...
    async def health() -> dict[str, Any]:
        ...
        try:
            if hasattr(mem, "_store") and mem._store:
                conn = mem._store._get_connection()  # type: ignore[attr-defined]
    ...
    async def submit_embed_batch_task(
        ...
        """
        effective_user = _resolve_user_id(request, req.user_id)
        from kemi.background_tasks import get_task_manager
    ...
    async def submit_rebuild_fts_task(
        ...
        task_manager = get_task_manager()
        try:
            task_id = task_manager.submit_rebuild_fts_index(user_id=target_user)
            return {"task_id": task_id, "status": "pending"}
        except RuntimeError as err:
            raise HTTPException(status_code=429, detail=str(err)) from err
    ...
    async def list_tasks(
        ...
        """
        from kemi.background_tasks import TaskStatus, get_task_manager
    ...
    async def admin_fts_stats(
        user_id: str | None = None, request: Request = None  # type: ignore[assignment]
    ) -> dict[str, Any]:
    ...
    async def admin_fts_verify(req: AdminFTSRepairRequest) -> dict[str, Any]:
    ...
    async def create_api_key(
        ...
        from kemi.api_keys import make_expiry
    ...
    async def get_memory_history(memory_id: str, request: Request, limit: int = 100) -> dict[str, Any]:
    ...
    def _get_webhook_store() -> WebhookStore | None:
        ...
        try:
            db_path = mem._store._db_path  # type: ignore[attr-defined]
    ...
    async def admin_list_users(request: Request) -> dict[str, Any]:
        ...
        for uid in users:
            ...
            # Optional metadata — best effort, never fail the response.
