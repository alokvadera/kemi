...
from kemi.interfaces.cli.writer import Writer, make_writer
...
def main() -> None:
    ...
    store_parser.add_argument(
        "--chunk",
        action="store_true",
        help="Automatically split content into semantic chunks before storing",
    )
    store_parser.add_argument(
        "--max-chunk-tokens",
        type=int,
        default=256,
        help="Max tokens per chunk (used with --chunk)",
    )
    store_parser.add_argument(
        "--chunk-overlap",
        type=int,
        default=1,
        help="Sentence overlap between chunks (used with --chunk)",
    )
    ...
    # chunk
    chunk_parser = subparsers.add_parser("chunk", help="Preview how content would be semantically chunked")  # noqa: E501
    ...
    # decompose
    decompose_parser = subparsers.add_parser("decompose", help="Decompose a query into sub-queries and show fusion")  # noqa: E501
    ...
    # rerank
    rerank_parser = subparsers.add_parser("rerank", help="Rerank recall results with cross-encoder scoring")  # noqa: E501
    ...
    rerank_parser.add_argument(
        "--provider",
        default="fallback",
        help="Reranker provider (fallback/nomic)",
    )
    ...
    webhook_add_parser.add_argument(
        ...
        help="Comma-separated event types: remembered,updated,forgotten,deleted,conflict,consolidated",  # noqa: E501
    )
    ...
    webhook_sub.add_parser("list", help="List registered webhooks")
    ...
    # rollback
    rollback_parser = subparsers.add_parser("rollback", help="Rollback a memory to a previous version")  # noqa: E501
...
def store_memory(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "chunk", False):
        ...
        from kemi.memory.chunker import semantic_chunks
        ...
        if not chunks:
            get_writer().error("chunking produced no results")
            sys.exit(1)
        if len(chunks) == 1:
            ...
            from kemi.memory.chunker import CHUNK_META_KEY
            parent_id = None
...
def update_memory(args: argparse.Namespace) -> None:
    if (
        ...
        get_writer().error("must specify at least one field to update")
        sys.exit(1)
    ...
    tags = [t.strip() for t in getattr(args, "tags", None).split(",")] if getattr(args, "tags", None) else None  # noqa: E501
...
def decompose_and_recall(args: argparse.Namespace) -> None:
    ...
    from kemi.nlp.decomposer import decompose_query, fused_recall
...
def rerank_recall(args: argparse.Namespace) -> None:
    ...
    from kemi.nlp.reranker import RerankerConfig, rerank_results
...
def rollback_memory(args: argparse.Namespace) -> None:
    ...
    if target_version is None:
        get_writer().error("missing --to-version argument")
        return
    ...
    if result:
        get_writer().write(f"Rolled back {args.memory_id} from v{result.from_version} to v{result.to_version}")  # noqa: E501
        get_writer().write(f"  Memory restored to: {target.content[:60]}...")
    else:
        get_writer().write("Rollback failed.")
def _get_cli_db_path() -> str:
    """Return the database path used by CLI commands, honouring KEMI_DB_PATH."""
    return os.environ.get("KEMI_DB_PATH", os.path.expanduser("~/.kemi/memories.db"))


def webhook_add(args: argparse.Namespace) -> None:
    """Register a new webhook endpoint."""
    from kemi.infra.webhooks import WebhookConfig, WebhookEventType, WebhookStore, validate_webhook_url
    ...
    try:
        validate_webhook_url(args.url)
    except ValidationError as e:
        get_writer().error(f"invalid webhook URL: {e}")
        return
...
def webhook_list(args: argparse.Namespace) -> None:
    ...
    from kemi.infra.webhooks import WebhookStore
...
def preview_chunk(args: argparse.Namespace) -> None:
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        get_writer().write(f"Chunk {i}/{len(chunks)} [{chunk.token_count_estimate()} tokens, strength={strength}]")  # noqa: E501
