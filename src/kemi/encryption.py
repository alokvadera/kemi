...
class EncryptionConfig:
    ...
    def key(self) -> str:
        ...
        raise ValueError("No encryption key configured. Set KEMI_ENCRYPTION_KEY env var or pass --key-file")
...
class FernetEncryptor:
    ...
    def __init__(self, key: str) -> None:
        ...
        # Derive a valid Fernet key using SHA-256 hash.
        # Fernet keys are 128-bit, base64-urlsafe encoded (43 bytes).
        # SHA-256 produces 32 bytes; we base64-encode directly to get a valid key.
        import hashlib
...
class FieldEncryptor:
    ...
    def __init__(
        ...
        self._encrypt_user_id = encrypt_user_id if encrypt_user_id is not None else getattr(config, "encrypt_user_id", False)
        self._encrypt_session_id = encrypt_session_id if encrypt_session_id is not None else getattr(config, "encrypt_session_id", False)
    ...
    def decrypt_memory_row(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        if self._encrypt_session_id and "session_id" in result and self._is_encrypted(result["session_id"]):
...
class SQLCipherManager:
    ...
    def __init__(self, key: str | None = None, key_file: str | None = None) -> None:
        if key is None and key_file is None:
            raise ValueError("SQLCipher requires a key (key= or key_file=)")
        if key_file:
