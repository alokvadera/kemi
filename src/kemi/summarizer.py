...
class LLMSummarizer:
    ...
    Supports four modes via the *provider* argument:
    ...
    def _init_client(self) -> None:
        ...
        if self._provider == "custom":
            if self._custom_callback is None:
                raise ValueError("custom_callback is required when provider='custom'")
            return

        if self._provider == "openai" or self._provider == "ollama":
            try:
                from openai import OpenAI
            except ImportError as exc:
                raise ImportError(
                    "openai package is required for provider='openai' or "
                    "provider='ollama'. Install with: pip install openai"
                ) from exc
            ...
            self._effective_model = self._model or (
                "gpt-4o-mini" if self._provider == "openai" else "llama3.2"
            )
            ...
            raise ValueError(
                ...
                "Expected one of: openai, anthropic, ollama, custom"
            )
        ...
        try:
            ...
            if self._provider in ("openai", "ollama"):
                ...
                response = self._client.chat.completions.create(
                    model=self._effective_model or "gpt-4o-mini",
                    messages=[
